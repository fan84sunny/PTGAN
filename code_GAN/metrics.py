...
class R1_mAP_eval():
    ...
    def update2(self, output):  # called once for each batch
        pid, camid, trackid = output
        self.pids.extend(np.asarray(pid))
        self.camids.extend(np.asarray(camid))
        self.tids.extend(np.asarray(trackid))
        self.unique_tids = list(set(self.tids))
    ...
    def track_ranking(self, qf, gf, gallery_tids, unique_tids, fic=False):
        ...
        # /home/ANYCOLOR2434/AICITY2021_Track2_DMT/AICITY2021_Track2_DMT-main/track_cam_rk.npy
    ...
    def compute(self, fic=False, fac=False, rm_camera=False, save_dir='./', crop_test=False, la=0.18):
        ...
        if fic:
            qf1, gf1, P, neg_vec = run_fic(self.qf, self.gf, self.q_camids, self.g_camids, None, None, la=la)
        ...
        if self.reranking_track:
            ...
            if fic:
                distmat1, origin_track_dist1 = self.track_ranking(qf1, gf1, self.gallery_tids, self.unique_tids, fic=True)
        ...
        # with open('/home/ANYCOLOR2434/AICITY2021_Track2_DMT/AICITY2021_Track2_DMT-main/logs/stage2/101a_384/veri_gan/pid2.txt', 'a+') as f:
        #     f.write(",")
        #     for i, garrley in enumerate(self.g_pids):
        #         f.write(str(garrley))
        #         if i < len(self.g_pids) - 1:
        #             f.write(',')
        #     f.write('\n')
        #     for i, query in enumerate(self.q_pids):
        #         f.write(str(query))
        #         for dis in distmat[i]:
        #             f.write(',' + str(dis))
        #         f.write('\n')
    ...
    def calcu_R1_mAP(self, distmat):
        # cam_matches = (self.g_camids == self.q_camids[:, np.newaxis]).astype(np.int32)
        # distmat = distmat + 10.0 * cam_matches
        # cam_matches = ((self.g_camids >= 40).astype(np.int32) != (self.q_camids[:, np.newaxis] >= 40).astype(np.int32)).astype(
        #     np.int32)
        # distmat = distmat + 10.0 * cam_matches
        ...
        # self.q_pids = np.asarray(self.pids[:self.num_query])
        # self.q_camids = np.asarray(self.camids[:self.num_query])
        # self.qf, self.gf = None, None
        ###
        cmc, mAP = eval_func(distmat, self.q_pids, self.g_pids, self.q_camids, self.g_camids)
        return cmc, mAP, distmat, self.pids, self.camids, self.qf, self.gf
...
class R1_mAP_eval_gen:
    ...
    def track_ranking(self, qf, gf, gallery_tids, unique_tids, fic=False):
        ...
        # cam_dist = np.load('/home/ANYCOLOR2434/AICITY2021_Track2_DMT/AICITY2021_Track2_DMT-main/track_cam_rk.npy')
        # view_dist = np.load('/home/ANYCOLOR2434/AICITY2021_Track2_DMT/AICITY2021_Track2_DMT-main/track_view_rk.npy')
        # track_dist = origin_track_dist - 0.1 * cam_dist - 0.05 * view_dist
        track_dist = origin_track_dist
        for i, tid in enumerate(gf_tids):
            dist[:, gallery_tids == tid] = track_dist[:, i:(i + 1)]
    ...
    def compute(self, la=0.18):
        ...
        # print("distmat1 size : ",distmat1.shape)
