...
from code_GAN.dataset_for_test import ImageDataset
...
def do_inference(cfg, model, query_loader, gallery_loader, gallery_pose_list):
    ...
    with torch.no_grad():
        for batch, gallery_data in enumerate(tqdm.tqdm(gallery_loader)):
            ...
            img = gallery_img
            gallary_paths.append(gallery_data['file_name'][0])
            # count===
            ...
            for query_data in query_loader:
                if batch == 0:
                    query_paths.append(query_data['file_name'][0])
                ...
                query_paths.extend(query_data['file_name'][0])
        ...
        # Compute topK image
        ### using topK and dictionary to ranking image, all query can rank gallery image
        # distmat_torch = torch.from_numpy(distmat)
        # values, indices = distmat_torch.topK(10, dim=1, largest=False, sorted=True)
        # dict_from_gallery = dict(zip(gallary_paths, [i for i in range(len(gallary_paths))]
        # dict_from_query = dict(zip(query_paths, [i for i in range(len(query_paths))]
        # indices_list = indices.tolist()
        # for index_query, i in enumerate(indices.tolist()):
        #    for index_gallery, j in enumerate(i):
        #       indices_list[index_query][index_gallery] = dict_from_gallery[j]
        #

        df = pd.DataFrame(distmat,index=query_paths,columns=gallary_paths)
        # similiar_img = df.sort_values(by=query_path, axis=1).loc[query_path]
        # sort_img_path = list(similiar_img)
        df.to_csv(os.path.join(cfg.OUTPUT_DIR, "similiar_img_distmat.csv"))
